...
import { refreshBotSettings } from "@/lib/telegram"
  ...
  try {
    ...
    await db.collection("settings").updateOne(
      { type: "bot_settings" },
      {
        $set: {
          ...settings,
          type: "bot_settings", // Ensure type is set
          updatedAt: new Date(),
        },
      },
      { upsert: true },
    ...
    // Immediately refresh bot settings to apply changes
    await refreshBotSettings()

    console.log("Settings saved and bot settings refreshed")
